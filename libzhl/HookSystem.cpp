...
int VariableDefinition::Load()
{
	...
	if (_useOffset)
	{
		/* shamelessly stolen from https://github.com/FTL-Hyperspace/FTL-Hyperspace/blob/master/zhl.cpp
...
// FunctionDefinition
 ...
 FunctionDefinition::FunctionDefinition(const char *name, const type_info &type, const char *sig, const HookSystem::ArgData *argdata, int nArgs, unsigned int flags, void **outfunc, const char* module) :
 	_sig(sig),
 	_argdata(argdata),
 	_nArgs(nArgs),
 	_flags(flags),
 ...
 FunctionDefinition::FunctionDefinition(const char* name, const type_info& type, void* addr, const HookSystem::ArgData* argdata, int nArgs, unsigned int flags, void** outfunc, const char* module) :
 	_address(addr),
 	_argdata(argdata),
 	_nArgs(nArgs),
 	_flags(flags),
...
#else
    *_outInternalSuper = originalFunction;
#endif
